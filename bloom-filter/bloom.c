...
int bloom_lookup(struct bloom * this, char * item) {
  int hashOne = hash_string(item, BLOOM_HASH1, this->size);
  int hashTwo = hash_string(item, BLOOM_HASH2, this->size);
  if (bitset_lookup(this->bitset, hashOne)== 1 && bitset_lookup(this->bitset, hashTwo) == 1) {
      return 1;
  } else {
    return 0;
  }
...
void bloom_add(struct bloom * this, char * item) {
  int hashOne = hash_string(item, BLOOM_HASH1, this->size);
  int hashTwo = hash_string(item, BLOOM_HASH2, this->size);
  bitset_add(this->bitset, hashOne);
  bitset_add(this->bitset, hashTwo);
}
